...
use pgb_clone_orchestrator::{
    guarded_apply_with_grant, revert, GrantedApplyError, LiveRequest, RevertConn, RevertError,
    RevertReport, RevertRow, WriteKind,
};
...
// ===========================================================================
//  The real-PG18 ApplyConn (a focused single-int-PK UPDATE conn for accounts).
// ===========================================================================
...
impl<'a> PgApplyConn<'a> {
    ...
    fn read_xact_raw(&mut self) -> Result<BTreeMap<String, (i64, i64, i64)>, ApplyError> {
        let rows = self
            ...
            .map_err(|e| classify(&e, self.statement_timeout_ms))?;
...
fn classify(e: &postgres::Error, timeout_ms: u64) -> ApplyError {
...
// ===========================================================================
//  A real-PG18 RevertConn (UPDATE pre-image upsert) — proves reversibility.
// ===========================================================================
...
impl RevertConn for PgRevertConn<'_> {
    ...
    fn restore_update(&mut self, relation: &str, rows: &[RevertRow]) -> Result<u64, RevertError> {
        // The seed `accounts` row: id PK, owner, balance. Re-apply the captured
        // OLD values (PreimageUpsert) for this UPDATE inverse.
        let mut n = 0u64;
        for row in rows {
            let id = match &row.pk.values()[0] {
                PkValue::Int(i) => *i,
                other => return Err(RevertError::Backend(format!("bad pk {other:?}"))),
            };
            let owner = row
                .before_image
                .iter()
            ...
            let (Some(owner), Some(balance)) = (owner, balance) else {
                return Err(RevertError::Backend("missing pre-image cols".into()));
            };
            let updated = self
                .client
                .execute(
    ...
    fn restore_insert(&mut self, _relation: &str, _rows: &[RevertRow]) -> Result<u64, RevertError> {
        // This IT only exercises an UPDATE inverse (PreimageUpsert); a DELETE
        // inverse would call this. Fail-closed: an unexpected INSERT inverse here
        // is a bug, not a silent no-op.
        Err(RevertError::Backend(
            "restore_insert not expected for the UPDATE-only grant-apply IT".into(),
        ))
    }
